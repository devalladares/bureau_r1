...
import { STATES, DEBUG_SINGLE_STATE_MODE, DEBUG_STATE, PARAMS } from './constants.js';
...
import { handleTouchStarted, handleTouchMoved, handleTouchEnded, touchPositions } from './interactions.js';
...
function checkStateTransition() {
  ...
  if (timeInState >= duration) {
    ...
    if (currentState === STATES.LOADING) {
      ...
      nextState = STATES.GRID; // Loop back to GRID after FLOCK
    } else {
...
function draw() {
  ...
  for (let p of particles) {
    ...
    p.display(currentState, stateStartTime);
  }
...
function windowResized() {
  ...
  PARAMS.bottomBoundary = PARAMS.MOBILE ? window.innerHeight * 0.3 : window.innerHeight * 0.15;
...
// Attach p5.js lifecycle functions to the global scope
window.setup = setup;
window.draw = draw;
